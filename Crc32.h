...
namespace pigz {
  ...
  // store the calculated x^(n*2^k) modulo p(x) using key length
  static std::map<size_t, uint32_t> x2nmpMap;
  ...
  unsigned long crc32_comb(unsigned long crc1, unsigned long crc2,
    ...
    uint32_t x2nmp;
    auto it = x2nmpMap.find(len2);
    if (it != x2nmpMap.end()) {
      x2nmp = it->second;
    } else {
      x2nmp = x2nmodp(len2, 3);
      x2nmpMap[len2] = x2nmp;
    }
                                  
      return multmodp(x2nmp, crc1) ^ crc2;
  }
